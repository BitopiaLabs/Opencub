...
import {DEFAULT_SEARCH_RESULTS, MAX_SEARCH_RESULTS} from '@/shared/constants';
import type {OpenCubToolExport} from '@/shared/types/core';
import {jsonSchema, tool} from '@/shared/types/core';
import {searchProjectContents} from '@/shared/utils/file-search';
import {isValidFilePath} from '@/shared/utils/path-validation';
	...
	inputSchema: jsonSchema<SearchFileContentsArgs>({
		...
		properties: {
			...
			path: {
				type: 'string',
				description:
					'Directory to scope the search to (relative path, e.g., "src/components", "src/tools"). Only files within this directory will be searched.',
			},
			wholeWord: {
				type: 'boolean',
				description:
...
export const searchFileContentsTool: OpenCubToolExport = {
