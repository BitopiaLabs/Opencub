...
import {findMatchingPaths} from '@/shared/utils/file-search';
	...
	inputSchema: jsonSchema<FindFilesArgs>({
		...
		properties: {
			pattern: {
				type: 'string',
				description:
					'Glob pattern to match file and directory paths. Examples: "*.tsx" (all .tsx files), "src/**/*.ts" (recursive in src/), "*.{ts,tsx}" (multiple extensions), "package.json" (exact file), "*config*" (files containing "config"), "src/tools/*.ts" (specific directory)',
			},
			maxResults: {
				type: 'number',
				description:
...
export const findFilesTool: OpenCubToolExport = {
