 ...
 * - User-level configuration (~/.config/opencub/agents/)
 * - Project-level configuration (.opencub/agents/)
 */
...
import {logError, logWarning} from '@/shared/utils/message-queue';
...
export class SubagentLoader {
	...
	private getUserAgentsPath(): string {
		...
		if (platform === 'darwin') {
			// macOS: ~/Library/Preferences/opencub/agents/
			return path.join(
				os.homedir(),
				'Library',
				'Preferences',
		...
		if (platform === 'win32') {
			// Windows: %APPDATA%/opencub/agents/
			return path.join(
		...
		// Linux and others: ~/.config/opencub/agents/
		return path.join(os.homedir(), '.config', 'opencub', 'agents');
	}
	...
	private getProjectAgentsPath(): string {
		return path.join(this.projectRoot, '.opencub', 'agents');
	}
