...
test.serial('loads project-level agent without permissionMode', async t => {
	const tempDir = join(tmpdir(), `opencub-test-${Date.now()}`);
	const agentsDir = join(tempDir, '.opencub', 'agents');
...
// ============================================================================
// Gap #7: Project-level agent loading from .opencub/agents/
// ============================================================================

test.serial('loads project-level agents from .opencub/agents/', async t => {
	const tempDir = join(tmpdir(), `opencub-test-${Date.now()}`);
	const agentsDir = join(tempDir, '.opencub', 'agents');
	mkdirSync(agentsDir, {recursive: true});
