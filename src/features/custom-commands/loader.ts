...
import {parseCommandFile} from '@/features/custom-commands/parser';
import {getConfigPath} from '@/shared/config/paths';
import type {CommandResource, CustomCommand} from '@/shared/types/index';
...
export class CustomCommandLoader {
	...
	constructor(projectRoot: string = process.cwd()) {
		...
		// nosemgrep
		this.projectCommandsDir = join(projectRoot, '.opencub', 'commands'); // nosemgrep
	...
	/**
	 * Check for deprecated .opencub/skills directories and warn
	 */
	private checkDeprecatedSkillsDirs(): void {
		if (this.deprecationWarned) return;

		const projectSkillsDir = join(this.projectRoot, '.opencub', 'skills');
		const personalSkillsDir = join(getConfigPath(), 'skills');
		let warned = false;

		if (existsSync(projectSkillsDir)) {
			logError(
				'Skills have been merged into commands. Move your SKILL.md files from .opencub/skills/ to .opencub/commands/ and rename them.',
			);
			warned = true;
		}
		if (existsSync(personalSkillsDir)) {
			logError(
				'Skills have been merged into commands. Move your SKILL.md files from ~/.config/opencub/skills/ to ~/.config/opencub/commands/ and rename them.',
			);
			warned = true;
		}
