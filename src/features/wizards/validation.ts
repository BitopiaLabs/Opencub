import {TIMEOUT_PROVIDER_CONNECTION_MS} from '@/shared/constants';
import type {ProviderConfig, SdkProvider} from '@/shared/types/config';
import {isLocalURL} from '@/shared/utils/url-utils';
...
export function validateConfig(
	...
	if (providers.length === 0) {
		warnings.push(
			'No providers configured. OpenCub requires at least one provider to function.',
		);
	}
...
export function buildConfigObject(
	...
	if (Object.keys(mcpServers).length > 0) {
		config.opencub.mcpServers = Object.values(mcpServers).map(server => ({
			name: server.name,
