...
test('buildConfigObject: converts MCP servers from Record to Array', t => {
	...
	t.is(config.opencub.mcpServers!.length, 2);

	// Should contain all servers
	const serverNames = config.opencub.mcpServers!.map(s => s.name);
...
test('buildConfigObject: preserves all server fields in array format', t => {
	...
	const remoteServer = config.opencub.mcpServers!.find(
...
test('buildConfigObject: sets enabled flag for wizard-generated configs', t => {
	...
	const testServer = config.opencub.mcpServers!.find(
...
test('buildConfigObject: maintains providers configuration', t => {
	...
	t.is(config.opencub.providers[0].name, 'openai');
	t.is(config.opencub.providers[0].apiKey, 'sk-test-key');
	t.is(config.opencub.providers[1].name, 'ollama');
	t.is(config.opencub.providers[1].baseUrl, 'http://localhost:11434');
});
...
test('buildConfigObject: maintains backward compatibility with provider fields', t => {
	...
	const provider = config.opencub.providers[0];
...
test('buildConfigObject: correctly maps all transport types', t => {
	...
	const servers = config.opencub.mcpServers!;
