...
import type {ProviderConfig} from '@/shared/types/config.js';
...
test('buildConfigObject: builds correct config with providers only', t => {
	...
	t.truthy(config.opencub);
	t.is(config.opencub.providers.length, 1);
	t.is(config.opencub.providers[0].name, 'ollama');
	t.is(config.opencub.providers[0].baseUrl, 'http://localhost:11434');
	t.deepEqual(config.opencub.providers[0].models, ['llama2', 'codellama']);
	t.is(config.opencub.mcpServers, undefined);
});
...
test('buildConfigObject: includes MCP servers when provided', t => {
	...
	t.truthy(config.opencub.mcpServers);
	t.true(Array.isArray(config.opencub.mcpServers));
	const filesystemServer = config.opencub.mcpServers?.find(
		s => s.name === 'filesystem',
	);
...
test('buildConfigObject: includes apiKey when present', t => {
	...
	t.is(config.opencub.providers[0].apiKey, 'sk-test-key');
});
...
test('buildConfigObject: includes organizationId when present', t => {
	...
	t.is(config.opencub.providers[0].organizationId, 'org-123');
});
...
test('buildConfigObject: includes timeout when present', t => {
	...
	t.is(config.opencub.providers[0].timeout, 30000);
});
...
test('buildConfigObject: omits optional fields when not present', t => {
	...
	t.is(config.opencub.providers[0].apiKey, undefined);
	t.is(config.opencub.providers[0].organizationId, undefined);
	t.is(config.opencub.providers[0].timeout, undefined);
});
...
test('buildConfigObject: handles multiple providers', t => {
	...
	t.is(config.opencub.providers.length, 2);
	t.is(config.opencub.providers[0].name, 'ollama');
	t.is(config.opencub.providers[1].name, 'openai');
});
...
test('buildConfigObject: handles multiple MCP servers', t => {
	...
	t.is(Object.keys(config.opencub.mcpServers ?? {}).length, 2);
	const filesystemServer = config.opencub.mcpServers?.find(
		s => s.name === 'filesystem',
	);
	const githubServer = config.opencub.mcpServers?.find(
		s => s.name === 'github',
	);
	t.truthy(filesystemServer);
...
test('buildConfigObject: includes caCertPath when present', t => {
	...
	t.is(config.opencub.providers[0].caCertPath, '/tmp/custom-ca.pem');
});
...
test('buildConfigObject: includes sdkProvider when present', t => {
	...
	t.is(config.opencub.providers[0].sdkProvider, 'google');
});
...
test('buildConfigObject: omits sdkProvider when not present', t => {
	...
	t.is(config.opencub.providers[0].sdkProvider, undefined);
});
...
test('buildConfigObject: handles Gemini provider with all fields', t => {
	...
	t.is(config.opencub.providers[0].name, 'Gemini');
	t.is(config.opencub.providers[0].sdkProvider, 'google');
	t.is(config.opencub.providers[0].baseUrl, 'https://generativelanguage.googleapis.com/v1beta');
	t.is(config.opencub.providers[0].apiKey, 'test-api-key');
	t.deepEqual(config.opencub.providers[0].models, ['gemini-3-flash-preview', 'gemini-3-pro-preview']);
});
...
test('buildConfigObject: handles GitHub Copilot provider with sdkProvider github-copilot', t => {
	...
	const p = config.opencub.providers[0];
