...
} from '@/features/session/key-generator';
import type {Message} from '@/shared/types/core.js';
...
test('getCurrentStats handles unknown model with no context limit', async t => {
	// Unset OPENCUB_CONTEXT_LIMIT so it doesn't act as a fallback
	const originalEnv = process.env.OPENCUB_CONTEXT_LIMIT;
	delete process.env.OPENCUB_CONTEXT_LIMIT;
	...
	try {
		...
		if (originalEnv !== undefined) {
			process.env.OPENCUB_CONTEXT_LIMIT = originalEnv;
		}
	}
});
