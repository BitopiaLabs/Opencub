...
import type {SessionUsage, TokenBreakdown, UsageData} from '@/shared/types/usage.js';
import {
	addSession,
	clearUsageData,
...
function createTestDir(): string {
	const testDir = path.join(
		os.tmpdir(),
		`opencub-test-${Date.now()}-${Math.random().toString(36).substring(7)}`,
	);
...
test('migrates usage data from legacy config directory to app data directory', t => {
	...
	// When OPENCUB_CONFIG_DIR is set, getConfigPath() returns it directly.
	// So here we create a fake legacy config dir and point OPENCUB_CONFIG_DIR at it.
	const legacyConfigDir = path.join(os.tmpdir(), 'opencub-legacy-config');
	...
	// Point OPENCUB_CONFIG_DIR to our legacy config dir to simulate pre-change behavior
	process.env.OPENCUB_CONFIG_DIR = legacyConfigDir;
	...
	const appDataDir = path.join(appDataHome, 'opencub');
...
test('migration removes legacy file after successful migration', t => {
	// Create legacy file
	const legacyConfigDir = path.join(os.tmpdir(), 'opencub-legacy-config-2');
	fs.mkdirSync(legacyConfigDir, {recursive: true});
	const legacyFilePath = path.join(legacyConfigDir, 'usage.json');
	const legacyData: UsageData = {
...
test('migration skips when new file already exists', t => {
	...
	const legacyConfigDir = path.join(os.tmpdir(), 'opencub-legacy-config-3');
	fs.mkdirSync(legacyConfigDir, {recursive: true});
	const legacyFilePath = path.join(legacyConfigDir, 'usage.json');
	const legacyData: UsageData = {
...
test('migration handles missing legacy config directory gracefully', t => {
	// Don't create legacy directory, just set OPENCUB_CONFIG_DIR to non-existent path
	process.env.OPENCUB_CONFIG_DIR = path.join(
		os.tmpdir(),
		'opencub-nonexistent',
	);
...
test('migration handles corrupt legacy file gracefully', t => {
	...
	const legacyConfigDir = path.join(os.tmpdir(), 'opencub-legacy-config-4');
	fs.mkdirSync(legacyConfigDir, {recursive: true});
	const legacyFilePath = path.join(legacyConfigDir, 'usage.json');
...
test('migration preserves all session data fields', t => {
	const legacyConfigDir = path.join(os.tmpdir(), 'opencub-legacy-config-5');
	fs.mkdirSync(legacyConfigDir, {recursive: true});
	const legacyFilePath = path.join(legacyConfigDir, 'usage.json');
...
test('readUsageData returns empty data on read error', t => {
	...
	const configDir = path.join(dataHome, 'opencub');
