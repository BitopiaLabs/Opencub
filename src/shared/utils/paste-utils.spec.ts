import type {PastePlaceholderContent, PlaceholderContent} from '@/shared/types/hooks';
import {PlaceholderType} from '@/shared/types/hooks';
...
const testDir = join(tmpdir(), `opencub-paste-test-${Date.now()}`);
...
test('handlePaste respects custom threshold - high threshold prevents placeholder', t => {
	...
	const configPath = join(testDir, 'opencub-preferences.json');
	writeFileSync(
		configPath,
		JSON.stringify({
			opencub: {
