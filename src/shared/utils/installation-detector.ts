...
/**
 * Detects how OpenCub was installed by using multiple detection strategies.
 * Uses a combination of path inspection, environment variables, and file system markers.
 * An environment variable `OPENCUB_INSTALL_METHOD` can be used to override detection for testing.
...
export function detectInstallationMethod(): InstallationMethod {
	...
	const envOverride = safeProcess.env?.OPENCUB_INSTALL_METHOD;
	if (envOverride) {
		...
		logWarning(
			`Invalid OPENCUB_INSTALL_METHOD: "${envOverride}". Valid values: ${validMethods.join(
				', ',
			)}`,
		);
	}
