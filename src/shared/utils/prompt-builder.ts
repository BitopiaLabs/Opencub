...
import type {SystemPromptConfig, TuneConfig} from '@/shared/types/config';
import {TUNE_DEFAULTS} from '@/shared/types/config';
import type {DevelopmentMode} from '@/shared/types/core';
import {getLogger} from '@/shared/utils/logging';
import {getSubagentDescriptions} from '@/shared/utils/prompt-processor';
import {isNanoProfile, isSingleToolProfile} from '@/tools/tool-profiles';

const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);
const sectionsDir = join(__dirname, '../../../src/app/prompts/sections');
...
export function getLastBuiltPrompt(): string {
	return (
		lastBuiltPrompt ?? 'You are OpenCub, a terminal-based AI coding agent.'
	);
}
...
function resolveSystemPromptOverride(
	...
	if (override.file !== undefined) {
		// Path comes from the user's own agents.config.json (trusted config), same model as src/config/index.ts
