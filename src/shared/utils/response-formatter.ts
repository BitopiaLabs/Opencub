...
import type {ToolCall} from '@/shared/types/index';
...
async function extractToolCalls(
	...
	// Try XML parser
	try {
		const {XMLToolCallParser} = await import('@/tools/tool-calling/xml-parser');
	...
	if (allowMixedContent) {
		try {
			const {parseToolCalls} = await import('@/tools/tool-calling/tool-parser');
