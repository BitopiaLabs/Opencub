...
test.beforeEach(() => {
	...
	delete process.env.OPENCUB_INSTALL_METHOD;
...
test('detectFromPath: homebrew takes precedence over node_modules', t => {
	...
	const path =
		'/opt/homebrew/Cellar/opencub/1.0.0/lib/node_modules/opencub/dist';
	t.is(detectFromPath(path), 'homebrew');
});
...
test('detectInstallationMethod: ignores invalid env override and continues detection', t => {
	...
	process.env.OPENCUB_INSTALL_METHOD = 'invalid-method';
	...
	// test output which shows: "Invalid OPENCUB_INSTALL_METHOD: invalid-method"
});
