...
import {TIMEOUT_UPDATE_CHECK_MS} from '@/shared/constants';
import type {NpmRegistryResponse, UpdateInfo} from '@/shared/types/index';
import {logError} from '@/shared/utils/message-queue';
import {detectInstallationMethod} from './installation-detector';

const UPDATE_COMMANDS = {
	NPM: 'npm update -g opencub',
...
async function fetchLatestVersion(): Promise<string | null> {
	try {
		const response = await fetch('https://registry.npmjs.org/opencub/latest', {
			method: 'GET',
			headers: {
				Accept: 'application/json',
				'User-Agent': 'opencub-update-checker',
			},
			// Add timeout
