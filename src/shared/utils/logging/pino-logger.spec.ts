...
// Test utilities
const testLogDir = join(tmpdir(), `opencub-pino-test-${Date.now()}`);
...
test('logger handles correlation context', async t => {
	...
	const originalCorrelation = process.env.OPENCUB_CORRELATION_ENABLED;
	process.env.OPENCUB_CORRELATION_ENABLED = 'true';
	...
	if (originalCorrelation === undefined) {
		delete process.env.OPENCUB_CORRELATION_ENABLED;
	} else {
		process.env.OPENCUB_CORRELATION_ENABLED = originalCorrelation;
	}
});
...
test('logger handles correlation when disabled', async t => {
	...
	const originalCorrelation = process.env.OPENCUB_CORRELATION_ENABLED;
	process.env.OPENCUB_CORRELATION_ENABLED = 'false';
