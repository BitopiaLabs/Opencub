...
import {BUFFER_LOG_BYTES, INTERVAL_LOG_FLUSH_MS} from '@/shared/constants';
...
export function createProductionTransport(
	...
	return {
		...
		level: 'info',
		options: {
			file: join(logDir, 'opencub-%Y-%m-%d.log'), // nosemgrep
			frequency: 'daily',
			size: '100m',
...
export function createMultiTransport(): TransportTarget[] {
	...
	if (env === 'production' || process.env.OPENCUB_LOG_TO_FILE === 'true') {
...
export function createErrorTransport(
	...
	return {
		...
		level: 'error',
		options: {
			file: join(logDir, 'opencub-error-%Y-%m-%d.log'), // nosemgrep
			frequency: 'daily',
...
export function createAuditTransport(
	...
	return {
		...
		level: 'info',
		options: {
			file: join(logDir, 'opencub-audit-%Y-%m-%d.log'), // nosemgrep
			frequency: 'daily',
			size: '200m',
			dateFormat: 'yyyy-MM-dd',
			extension: '.log',
...
export function getTransportFromEnvironment():
	...
	const transportTypes = (process.env.OPENCUB_LOG_TRANSPORTS || 'default')
		.split(',')
		.map(t => t.trim());
	const transports: TransportTarget[] = [];
