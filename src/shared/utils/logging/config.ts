 ...
 * - Linux: ~/.local/state/opencub/logs (XDG_STATE_HOME)
 * - Windows: %LOCALAPPDATA%/opencub/logs
 */
export function getDefaultLogDirectory(): string {
	if (process.env.OPENCUB_LOG_DIR) {
		return process.env.OPENCUB_LOG_DIR;
	}
	...
	switch (platform()) {
		...
		case 'darwin':
			return join(homedir(), 'Library', 'Logs', 'opencub');
		default: // linux
			return join(
...
function createDevelopmentConfig(): EnhancedLoggerConfig {
	return {
		level: (process.env.OPENCUB_LOG_LEVEL as LogLevel) || 'debug',
...
function createProductionConfig(): EnhancedLoggerConfig {
	...
	const disableFileLogging = process.env.OPENCUB_LOG_DISABLE_FILE === 'true';

	// File log level defaults to 'info' for useful diagnostics
	// Can be overridden with OPENCUB_LOG_LEVEL env var
	const fileLogLevel =
		(process.env.OPENCUB_LOG_LEVEL as LogLevel) ||
	...
	return {
		...
		// when OPENCUB_LOG_TO_FILE=true is set
	};
}
 ...
 * This gives users a clean experience. Developers working on opencub itself should
