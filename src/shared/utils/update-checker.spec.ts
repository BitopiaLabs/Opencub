...
test('checkForUpdates: returns correct update command', async t => {
	...
	t.is(result.updateCommand, 'npm update -g opencub');
});
...
test('checkForUpdates: returns correct Homebrew command when installed via Homebrew', async t => {
	...
	t.is(
		...
		'brew list opencub >/dev/null 2>&1 && brew upgrade opencub || (echo "Error: opencub not found in Homebrew. Please install it first with: brew install opencub" && exit 1)',
	);
});
