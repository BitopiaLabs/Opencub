...
test.serial('uses OPENCUB_DEFAULT_SHUTDOWN_TIMEOUT env var', (t) => {
	const originalEnv = process.env.OPENCUB_DEFAULT_SHUTDOWN_TIMEOUT;

	process.env.OPENCUB_DEFAULT_SHUTDOWN_TIMEOUT = '10000';
	const manager = getShutdownManager();
	t.is(manager['timeoutMs'], 10000);

	process.env.OPENCUB_DEFAULT_SHUTDOWN_TIMEOUT = originalEnv ?? '';
	resetShutdownManager();
});
...
test.serial('programmatic timeoutMs takes priority over env var', (t) => {
	...
	process.env.OPENCUB_DEFAULT_SHUTDOWN_TIMEOUT = '5000';
...
test.serial('invalid env var falls back to default', (t) => {
	...
	process.env.OPENCUB_DEFAULT_SHUTDOWN_TIMEOUT = 'invalid';
	const manager = getShutdownManager();
