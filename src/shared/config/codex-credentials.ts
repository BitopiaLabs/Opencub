 ...
 * Stored under config path (e.g. ~/.config/opencub/) so they are not in project config.
 */
...
import type {CodexTokens} from '@/features/auth/chatgpt-codex';
...
export function getCodexNoCredentialsMessage(providerName: string): string {
	return `No Codex credentials for "${providerName}". Type /codex-login in the chat to log in, or run: opencub codex login`;
}
