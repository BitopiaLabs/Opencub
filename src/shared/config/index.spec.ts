...
test('loadAppConfig handles malformed JSON gracefully', async t => {
	...
	writeFileSync(configPath, '{ "opencub": { "providers": [ }, "mcpServers": [ ] }', 'utf-8');
...
const defaultModeTestDir = join(
	tmpdir(),
	`opencub-default-mode-test-${Date.now()}`,
);
...
test.serial('loadDefaultMode returns undefined when no config exists', async t => {
	const originalCwd = process.cwd();
	const originalEnv = process.env.OPENCUB_CONFIG_DIR;

	try {
		process.chdir(defaultModeTestDir);
		process.env.OPENCUB_CONFIG_DIR = join(defaultModeTestDir, 'empty-config');
		...
		if (originalEnv !== undefined) {
			process.env.OPENCUB_CONFIG_DIR = originalEnv;
		} else {
		...
		async t => {
			...
			try {
				writeFileSync(
					...
					JSON.stringify({opencub: {defaultMode: mode}}),
					'utf-8',
				);
				process.chdir(testSubdir);
				process.env.OPENCUB_CONFIG_DIR = join(testSubdir, 'nonexistent-global');
...
test.serial('loadDefaultMode prefers project config over global config', async t => {
	...
	try {
		writeFileSync(
			...
			JSON.stringify({opencub: {defaultMode: 'yolo'}}),
			'utf-8',
		);
		writeFileSync(
			join(globalDir, 'agents.config.json'),
			JSON.stringify({opencub: {defaultMode: 'plan'}}),
			'utf-8',
		);
...
const systemPromptTestDir = join(
	tmpdir(),
	`opencub-system-prompt-test-${Date.now()}`,
);
...
test.serial('loadDefaultMode returns undefined for invalid mode values', async t => {
	...
	try {
		writeFileSync(
			join(testSubdir, 'agents.config.json'),
			JSON.stringify({opencub: {defaultMode: 'invalid-mode'}}),
			'utf-8',
		);
		process.chdir(testSubdir);
		process.env.OPENCUB_CONFIG_DIR = join(testSubdir, 'nonexistent-global');
