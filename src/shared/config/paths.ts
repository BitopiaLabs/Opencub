...
export function getAppDataPath(): string {
	...
	if (process.env.OPENCUB_DATA_DIR) {
		return process.env.OPENCUB_DATA_DIR;
	}

	// Check XDG_DATA_HOME first (works cross-platform for testing)
	if (process.env.XDG_DATA_HOME) {
		return join(process.env.XDG_DATA_HOME, 'opencub');
	}
	...
	return join(baseAppDataPath, 'opencub');
}

export function getConfigPath(): string {
	// Allow explicit override via environment variable
	if (process.env.OPENCUB_CONFIG_DIR) {
		return process.env.OPENCUB_CONFIG_DIR;
	}
	...
	return join(baseConfigPath, 'opencub');
}
