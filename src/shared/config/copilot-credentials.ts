 ...
 * (e.g. ~/.config/opencub/) so they are not in project config.
 */
...
import {clearCopilotTokenCache} from '@/features/auth/github-copilot';
...
export function getCopilotNoCredentialsMessage(providerName: string): string {
	return `No Copilot credentials for "${providerName}". Type /copilot-login in the chat to log in, or run: opencub copilot login (from project: node dist/cli.js copilot login)`;
}
