...
/**
 * Check if a opencub tool is configured to always be allowed.
 * @param toolName - The name of the tool to check
 * @returns true if the tool is in the top-level alwaysAllow list
 */
export function isOpenCubToolAlwaysAllowed(toolName: string): boolean {
...
export function getBraveSearchApiKey(): string | undefined {
	const apiKey = getAppConfig().opencubTools?.webSearch?.apiKey;
