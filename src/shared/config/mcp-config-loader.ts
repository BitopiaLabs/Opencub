...
import type {MCPServerConfig, ProviderConfig} from '@/shared/types/config';
...
/**
 * Load global MCP configuration from ~/.config/opencub/.mcp.json
 */
...
function loadEnvMCPConfigs(): MCPServerWithSource[] {
	let rawData = process.env.OPENCUB_MCPSERVERS;

	if (!rawData && process.env.OPENCUB_MCPSERVERS_FILE) {
		if (existsSync(process.env.OPENCUB_MCPSERVERS_FILE)) {
			try {
				rawData = readFileSync(process.env.OPENCUB_MCPSERVERS_FILE, 'utf-8');
			} catch (error) {
				logError(
					`Failed to read OPENCUB_MCPSERVERS_FILE at ${process.env.OPENCUB_MCPSERVERS_FILE}: ${String(error)}`,
				);
			}
		}
	}
...
function loadProjectProviderConfigs(): ProviderConfig[] {
	...
	if (existsSync(configPath)) {
		try {
			...
			if (config.opencub && Array.isArray(config.opencub.providers)) {
				// Apply environment variable substitution
				const processedProviders = substituteEnvVars(config.opencub.providers);
...
function loadEnvProviderConfigs(): ProviderConfig[] {
	let rawData = process.env.OPENCUB_PROVIDERS;

	if (!rawData && process.env.OPENCUB_PROVIDERS_FILE) {
		if (existsSync(process.env.OPENCUB_PROVIDERS_FILE)) {
			try {
				rawData = readFileSync(process.env.OPENCUB_PROVIDERS_FILE, 'utf-8');
			} catch (error) {
				logError(
					`Failed to read OPENCUB_PROVIDERS_FILE at ${process.env.OPENCUB_PROVIDERS_FILE}: ${String(error)}`,
				);
			}
		}
	}
	...
	try {
		...
		} else if (config.opencub && Array.isArray(config.opencub.providers)) {
			return substituteEnvVars(config.opencub.providers);
		} else if (Array.isArray(config.providers)) {
