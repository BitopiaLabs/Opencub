...
import {loadAllMCPConfigs, loadGlobalMCPConfig, loadProjectMCPConfig, loadAllProviderConfigs, mergeMCPConfigs} from '@/shared/config/mcp-config-loader';
...
test('loadGlobalMCPConfig - loads from global .mcp.json', t => {
    ...
    const originalConfigDir = process.env.OPENCUB_CONFIG_DIR;
    try {
        process.env.OPENCUB_CONFIG_DIR = testDir;
        ...
        if (originalConfigDir !== undefined) {
            process.env.OPENCUB_CONFIG_DIR = originalConfigDir;
        } else {
            delete process.env.OPENCUB_CONFIG_DIR;
        }
    }
});
...
test('loadEnvMCPConfigs - loads from OPENCUB_MCPSERVERS environment variable', t => {
    const originalValue = process.env.OPENCUB_MCPSERVERS;
    ...
    try {
        ...
        process.env.OPENCUB_MCPSERVERS = JSON.stringify(config);
        ...
        if (originalValue !== undefined) {
            process.env.OPENCUB_MCPSERVERS = originalValue;
        } else {
            delete process.env.OPENCUB_MCPSERVERS;
        }
    }
});
...
test('loadEnvMCPConfigs - loads from OPENCUB_MCPSERVERS_FILE', t => {
    const testDir = t.context.testDir as string;
    const originalValue = process.env.OPENCUB_MCPSERVERS;
    const originalFileValue = process.env.OPENCUB_MCPSERVERS_FILE;
    ...
    try {
        ...
        process.env.OPENCUB_MCPSERVERS_FILE = filePath;
        delete process.env.OPENCUB_MCPSERVERS; // Ensure direct var doesn't interfere
        ...
        if (originalFileValue !== undefined) {
            process.env.OPENCUB_MCPSERVERS_FILE = originalFileValue;
        } else {
            delete process.env.OPENCUB_MCPSERVERS_FILE;
        }
    }
});
...
test('loadEnvMCPConfigs - handles invalid JSON gracefully', t => {
    ...
    try {
        process.env.OPENCUB_MCPSERVERS = 'invalid json {';
...
test('loadEnvProviderConfigs - loads from OPENCUB_PROVIDERS environment variable', t => {
    const originalValue = process.env.OPENCUB_PROVIDERS;
    ...
    try {
        ...
        process.env.OPENCUB_PROVIDERS = JSON.stringify(providers);
        ...
        if (originalValue !== undefined) {
            process.env.OPENCUB_PROVIDERS = originalValue;
        } else {
            delete process.env.OPENCUB_PROVIDERS;
        }
    }
});

test('loadEnvProviderConfigs - supports opencub wrapper format', t => {
    const originalValue = process.env.OPENCUB_PROVIDERS;
    ...
    try {
        ...
        process.env.OPENCUB_PROVIDERS = JSON.stringify(config);
...
test('hierarchical precedence - env overrides project and global', t => {
    const testDir = t.context.testDir as string;
    const originalMcpServers = process.env.OPENCUB_MCPSERVERS;
    const originalConfigDir = process.env.OPENCUB_CONFIG_DIR;
    ...
    try {
        ...
        process.env.OPENCUB_CONFIG_DIR = globalDir;
        const globalConfig = {
            mcpServers: {
                'override-test': {
                    transport: 'stdio',
        ...
        process.env.OPENCUB_MCPSERVERS = JSON.stringify(envConfig);
        ...
        if (originalMcpServers !== undefined) {
            process.env.OPENCUB_MCPSERVERS = originalMcpServers;
        } else {
            delete process.env.OPENCUB_MCPSERVERS;
        }
        if (originalConfigDir !== undefined) {
...
test('hierarchical precedence - OPENCUB_PROVIDERS overrides all', t => {
    const testDir = t.context.testDir as string;
    const originalProviders = process.env.OPENCUB_PROVIDERS;
    const originalConfigDir = process.env.OPENCUB_CONFIG_DIR;
    const originalNodeEnv = process.env.NODE_ENV;
    ...
    try {
        ...
        process.env.OPENCUB_PROVIDERS = JSON.stringify(envConfig);
        ...
        if (originalProviders !== undefined) {
            process.env.OPENCUB_PROVIDERS = originalProviders;
        } else {
            delete process.env.OPENCUB_PROVIDERS;
        }
        if (originalConfigDir !== undefined) {
