...
const testConfigDir = join(tmpdir(), `opencub-test-config-${Date.now()}`);

test.before(() => {
	// Set config directory for all tests
	process.env.OPENCUB_CONFIG_DIR = testConfigDir;
...
const getTestPreferencesPath = () => join(testConfigDir, 'opencub-preferences.json');
...
// ============================================================================
// updateOpenCubShape Tests
// ============================================================================

test.serial('updateOpenCubShape saves opencub shape', t => {
	const preferencesPath = getTestPreferencesPath();
	if (existsSync(preferencesPath)) {
		rmSync(preferencesPath, {force: true});
	}
	...
	try {
		...
		t.is(parsed.opencubShape, 'block');
	} finally {
		if (existsSync(preferencesPath)) {
			rmSync(preferencesPath, {force: true});
		}
...
test.serial('updateOpenCubShape preserves existing preferences', t => {
	const preferencesPath = getTestPreferencesPath();
	const existingPreferences: UserPreferences = {
		lastProvider: 'openrouter',
		lastModel: 'claude-3-opus',
	...
	try {
		updateOpenCubShape('chrome');

		const content = readFileSync(preferencesPath, 'utf-8');
		const parsed = JSON.parse(content) as UserPreferences;

		t.is(parsed.opencubShape, 'chrome');
		t.is(parsed.lastProvider, 'openrouter');
...
test.serial('updateOpenCubShape overwrites existing opencub shape', t => {
	const preferencesPath = getTestPreferencesPath();
	const existingPreferences: UserPreferences = {
		opencubShape: 'tiny',
	};
	...
	try {
		updateOpenCubShape('huge');

		const content = readFileSync(preferencesPath, 'utf-8');
		const parsed = JSON.parse(content) as UserPreferences;

		t.is(parsed.opencubShape, 'huge');
	} finally {
		if (existsSync(preferencesPath)) {
			rmSync(preferencesPath, {force: true});
		}
...
// ============================================================================
// getOpenCubShape Tests
// ============================================================================

test.serial('getOpenCubShape returns saved opencub shape', t => {
	const preferencesPath = getTestPreferencesPath();
	const preferences: UserPreferences = {
		opencubShape: 'slick',
	};
...
test.serial('getOpenCubShape returns undefined when not set', t => {
	const preferencesPath = getTestPreferencesPath();
	const preferences: UserPreferences = {};
	writeFileSync(preferencesPath, JSON.stringify(preferences, null, 2), 'utf-8');
...
test.serial('getOpenCubShape returns undefined when file does not exist', t => {
	const preferencesPath = getTestPreferencesPath();
	if (existsSync(preferencesPath)) {
		rmSync(preferencesPath, {force: true});
	}
...
test.serial('full workflow: update and retrieve opencub shape', t => {
	const preferencesPath = getTestPreferencesPath();
	if (existsSync(preferencesPath)) {
		rmSync(preferencesPath, {force: true});
	}
	...
	try {
		updateOpenCubShape('grid');
		const retrieved = getOpenCubShape();
		t.is(retrieved, 'grid');
	} finally {
		if (existsSync(preferencesPath)) {
			rmSync(preferencesPath, {force: true});
