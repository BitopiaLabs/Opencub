...
import {substituteEnvVars} from '@/shared/config/env-substitution';
import {
	loadAllMCPConfigs,
	loadAllProviderConfigs,
} from '@/shared/config/mcp-config-loader';
import {getConfigPath} from '@/shared/config/paths';
import {
	getNotificationsPreference,
	loadPreferences,
} from '@/shared/config/preferences';
import {defaultTheme, getThemeColors} from '@/shared/config/themes';
import type {
	AppConfig,
	AutoCompactConfig,
	Colors,
...
import {DEFAULT_SINGLE_LINE_PASTE_THRESHOLD} from '@/shared/utils/paste-utils';
...
export function getClosestConfigFile(fileName: string): string {
	try {
		...
		// If OPENCUB_CONFIG_DIR is explicitly set, skip cwd and home checks
		// and use only the config directory (important for tests and explicit overrides)
		const isExplicitConfigDir = Boolean(process.env.OPENCUB_CONFIG_DIR);
...
function tryLoadAutoCompactFromPath(
	...
	try {
		...
		const autoCompact = config.opencub?.autoCompact;
...
function tryLoadSessionsFromPath(
	...
	try {
		const rawData = readFileSync(configPath, 'utf-8');
		const config = JSON.parse(rawData);
		const sessions = config.opencub?.sessions;
...
function loadSessionConfig(): AppConfig['sessions'] {
	...
	// Try to load from project-level config first
	const projectConfigPath = join(process.cwd(), 'opencub-preferences.json');
	...
	// Try global config
	const configDir = getConfigPath();
	const globalConfigPath = join(configDir, 'opencub-preferences.json');
...
function tryLoadPasteFromPath(
	...
	try {
		const rawData = readFileSync(configPath, 'utf-8');
		const config = JSON.parse(rawData);
		const paste = config.opencub?.paste;
...
function loadOpenCubToolsConfig(): AppConfig['opencubTools'] {
	// Try project-level config first
	const projectConfigPath = join(process.cwd(), 'agents.config.json');
	const projectResult = tryLoadOpenCubToolsFromPath(projectConfigPath);
	if (projectResult) {
	...
	const configDir = getConfigPath();
	const globalConfigPath = join(configDir, 'agents.config.json');
	return tryLoadOpenCubToolsFromPath(globalConfigPath) ?? undefined;
}

function tryLoadOpenCubToolsFromPath(
	configPath: string,
): AppConfig['opencubTools'] | null {
	if (!existsSync(configPath)) {
		return null;
	}
	...
	try {
		const rawData = readFileSync(configPath, 'utf-8');
		const config = JSON.parse(rawData);
		const opencubTools = config.opencub?.opencubTools;
		if (opencubTools && typeof opencubTools === 'object') {
			return substituteEnvVars(opencubTools);
		}
	} catch (error) {
		logError(
			`Failed to load opencubTools config from ${configPath}: ${String(error)}`,
		);
	}
...
function tryLoadAlwaysAllowFromPath(configPath: string): string[] | null {
	...
	try {
		const rawData = readFileSync(configPath, 'utf-8');
		const config = JSON.parse(rawData);
		const alwaysAllow = config.opencub?.alwaysAllow;
...
function tryLoadSystemPromptFromPath(
	...
	try {
		const rawData = readFileSync(configPath, 'utf-8');
		const config = JSON.parse(rawData);
		const systemPrompt = config.opencub?.systemPrompt;
...
function tryLoadDisabledToolsFromPath(configPath: string): string[] | null {
	...
	try {
		const rawData = readFileSync(configPath, 'utf-8');
		const config = JSON.parse(rawData);
		const disabledTools = config.opencub?.disabledTools;
...
function tryLoadDefaultModeFromPath(configPath: string): CliMode | null {
	...
	try {
		const rawData = readFileSync(configPath, 'utf-8');
		const config = JSON.parse(rawData);
		const defaultMode = config.opencub?.defaultMode;
...
function loadAppConfig(): AppConfig {
	...
	// Load opencub tools configuration
	const opencubTools = loadOpenCubToolsConfig();
