...
// These tests intentionally lock in the public contract for OpenCub's
...
test.serial('getAppDataPath uses OPENCUB_DATA_DIR override verbatim', t => {
	testPathGetter(
		t,
		'linux',
		{
...
test.serial('getAppDataPath darwin default path is stable', t => {
	testPathGetter(
		...
		{HOME: '/Users/test'},
		getAppDataPath,
		path.join('/Users/test', 'Library', 'Application Support', 'opencub'),
	);
});
...
test.serial('getAppDataPath win32 uses APPDATA when set', t => {
	testPathGetter(
		...
		getAppDataPath,
		path.join('C:', 'Users', 'test', 'AppData', 'Roaming', 'opencub'),
	);
});
...
test.serial('getAppDataPath linux falls back to ~/.local/share', t => {
	testPathGetter(
		...
		getAppDataPath,
		path.join('/home/test', '.local', 'share', 'opencub'),
	);
});
...
test.serial('getConfigPath uses OPENCUB_CONFIG_DIR override verbatim', t => {
	testPathGetter(
		t,
		'linux',
		{
...
test.serial('getConfigPath darwin default path is stable', t => {
	testPathGetter(
		t,
		'darwin',
		{OPENCUB_CONFIG_DIR: undefined, HOME: '/Users/test'},
		getConfigPath,
		path.join('/Users/test', 'Library', 'Preferences', 'opencub'),
	);
});
...
test.serial('getConfigPath linux falls back to ~/.config', t => {
	testPathGetter(
		...
		getConfigPath,
		path.join('/home/test', '.config', 'opencub'),
	);
});
