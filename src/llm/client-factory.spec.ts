...
import {clearAppConfig, reloadAppConfig} from '@/shared/config/index';
import {resetPreferencesCache} from '@/shared/config/preferences';
...
const testDir = join(tmpdir(), `opencub-client-factory-test-${Date.now()}`);
...
test.serial(
	...
	async t => {
		...
		writeFileSync(
			join(configDir, 'opencub-preferences.json'),
