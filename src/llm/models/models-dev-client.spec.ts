...
// ============================================================================
// OPENCUB_CONTEXT_LIMIT Environment Variable
// ============================================================================

test('getModelContextLimit - env variable used for unknown models', async t => {
	const saved = process.env.OPENCUB_CONTEXT_LIMIT;
	process.env.OPENCUB_CONTEXT_LIMIT = '32000';
	try {
		const limit = await getModelContextLimit('unknown-model-12345');
		t.is(limit, 32000);
	} finally {
		delete process.env.OPENCUB_CONTEXT_LIMIT;
		if (saved !== undefined) {
			process.env.OPENCUB_CONTEXT_LIMIT = saved;
		}
	}
});
