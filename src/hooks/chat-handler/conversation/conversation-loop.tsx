...
import {MAX_EMPTY_TURNS, MAX_MALFORMED_RETRIES} from '@/shared/constants';
...
import {performAutoCompact} from '@/shared/utils/auto-compact';
import {
	formatElapsedTime,
	getRandomAdjective,
} from '@/shared/utils/completion-note';
import {MessageBuilder} from '@/shared/utils/message-builder';
import {parseToolArguments} from '@/shared/utils/tool-args-parser';
import {toolNeedsApproval} from '@/shared/utils/tool-needs-approval';
import {displayCompactCountsSummary} from '@/shared/utils/tool-result-display';
...
export const processAssistantResponse = async (
	...
	const flushLiveTaskList = async () => {
		...
		if (tasks.length > 0) {
			const {TaskListDisplay} = await import('@/ui/task-list-display');
			addToChatQueue(
				<TaskListDisplay
