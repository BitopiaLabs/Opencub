...
import {getThemeColors, themes} from '@/shared/config/themes';
import type {NotificationsConfig} from '@/shared/types/config';
import type {OpenCubShape, ThemePreset} from '@/shared/types/ui';
import {setNotificationsConfig} from '@/shared/utils/notifications';
...
function ThemeMiniPreview({
	...
	return (
		<Box flexDirection="column">
			...
			<Box flexDirection="column" marginBottom={compact ? 0 : 1}>
				<Text color={colors.tool}>⚒ read_file src/app.tsx</Text>
...
// OpenCub Shape settings panel
function SettingsOpenCubShapePanel({
	onBack,
	onCancel,
}: {
	onBack: () => void;
	...
	const savedShape = getOpenCubShape();
	const initialShape: OpenCubShape = savedShape ?? 'tiny';
	const [originalShape] = useState<OpenCubShape>(initialShape);
	const [previewShape, setPreviewShape] = useState<OpenCubShape>(initialShape);
	...
	const shapeOptions: {label: string; value: OpenCubShape}[] = useMemo(
		() => [
	...
	const handleSelect = (item: {label: string; value: OpenCubShape}) => {
		updateOpenCubShape(item.value);
		onBack();
	};

	const handleHighlight = (item: {label: string; value: OpenCubShape}) => {
		setPreviewShape(item.value);
	};

	const displayText = isNarrow ? 'NC' : 'OpenCub';
