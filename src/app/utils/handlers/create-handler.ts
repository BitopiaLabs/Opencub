...
import type {SubagentConfigWithSource} from '@/features/subagents/types';
import type {MessageSubmissionOptions} from '@/shared/types/index';
import {ErrorMessage, SuccessMessage} from '@/ui/message-box';
...
async function handleFileCreate(
	...
	const targetDir = join(process.cwd(), '.opencub', dirName);
	...
	if (existsSync(filePath)) {
		onAddToChatQueue(
			React.createElement(ErrorMessage, {
				...
				message: `${entityName === 'schedule' ? 'Schedule' : 'Command'} file already exists: .opencub/${dirName}/${safeName}`,
			}),
		);
		onCommandComplete?.();
		return true;
...
`;
	...
	onAddToChatQueue(
		React.createElement(SuccessMessage, {
			...
			message: `Created ${entityName} file: .opencub/${dirName}/${safeName}`,
			hideBox: true,
		}),
	);
...
export async function handleScheduleCreate(
	...
	return handleFileCreate(
		...
		'schedule',
		safeName =>
			`I just created a new schedule command file at .opencub/schedules/${safeName}. Help me write the content for this scheduled task. Ask me what I want this scheduled job to do, then write the markdown prompt into the file using the write_file tool. The file should contain a clear prompt that instructs the AI agent what to do when this schedule runs. Keep the YAML frontmatter at the top with the description field.`,
		options,
	);
}
...
export async function handleAgentCreate(
	...
	const safeName = fileName.endsWith('.md') ? fileName : `${fileName}.md`;
	const targetDir = join(process.cwd(), '.opencub', 'agents');
	const filePath = join(targetDir, safeName);
	...
	if (existsSync(filePath)) {
		onAddToChatQueue(
			React.createElement(ErrorMessage, {
				...
				message: `Agent file already exists: .opencub/agents/${safeName}`,
			}),
		);
		onCommandComplete?.();
		return true;
...
`;
	...
	onAddToChatQueue(
		React.createElement(SuccessMessage, {
			...
			message: `Created agent file: .opencub/agents/${safeName}`,
			hideBox: true,
		}),
	);

	await onHandleChatMessage(
		`I just created a new subagent definition file at .opencub/agents/${safeName}. Help me write the content for this agent. Ask me what I want this agent to specialize in, then write the complete markdown file using the write_file tool.
 ...
 * .opencub/agents/ so it can be customized.
 * Returns true if handled.
 */
export async function handleAgentCopy(
	...
	onAddToChatQueue(
		React.createElement(SuccessMessage, {
			...
			message: `Copied agent '${agentName}' to .opencub/agents/${safeName}\nYou can now modify this file to customize the agent.`,
			hideBox: true,
		}),
	);
...
export async function handleCommandCreate(
	...
	return handleFileCreate(
		...
		(safeName, commandBaseName) =>
			`I just created a new custom command file at .opencub/commands/${safeName}. Help me write the content for this command. Ask me what I want this command to do, then write the markdown prompt into the file using the write_file tool. The file should contain a clear prompt that instructs the AI what to do when this command is invoked via /${commandBaseName}. Keep the YAML frontmatter at the top.
