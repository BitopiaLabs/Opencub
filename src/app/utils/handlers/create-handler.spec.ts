...
import {SubagentLoader, getSubagentLoader} from '@/features/subagents/subagent-loader.js';

console.log('\ncreate-handler.spec.ts');

// Create a temporary directory for each test
function createTempDir(): string {
	const dir = join(tmpdir(), `opencub-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
