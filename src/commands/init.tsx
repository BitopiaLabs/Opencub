...
import {AgentsTemplateGenerator} from '@/features/init/agents-template-generator';
import {ExistingRulesExtractor} from '@/features/init/existing-rules-extractor';
import {ProjectAnalyzer} from '@/features/init/project-analyzer';
import {generateKey} from '@/features/session/key-generator';
import {useTerminalWidth} from '@/hooks/useTerminalWidth';
import {getColors} from '@/shared/config/index';
import {Command} from '@/shared/types/index';
import {ErrorMessage} from '@/ui/message-box';
		...
		>
			<Box marginBottom={1}>
				<Text color={colors.primary} bold>
					✓ OpenCub project initialized successfully!
				</Text>
			</Box>
...
export const initCommand: Command = {
	name: 'init',
	description:
		'Initialize opencub configuration and analyze project structure. Use --force to regenerate AGENTS.md, --lean to skip CLAUDE.md when generating AGENTS.md.',
	handler: (args: string[], _messages, _metadata) => {
		...
		try {
			...
			const opencubDir = join(cwd, '.opencub');

			// Check for existing initialization
			const hasAgents = existsSync(agentsPath);
			const hasOpenCub = existsSync(opencubDir);

			if (hasAgents && hasOpenCub && !forceRegenerate) {
				return Promise.resolve(
					React.createElement(InitError, {
						key: generateKey('init-error'),
						message:
							'Project already initialized. Found AGENTS.md and .opencub/ directory. Use /init --force to regenerate.',
					}),
				);
			}
			...
			// Create .opencub directory structure
			if (!hasOpenCub) {
				mkdirSync(opencubDir, {recursive: true});
				created.push('.opencub/');
			}

			const commandsDir = join(opencubDir, 'commands');
			if (!existsSync(commandsDir)) {
				mkdirSync(commandsDir, {recursive: true});
				created.push('.opencub/commands/');
			}
			...
			for (const [filename, content] of Object.entries(exampleCommands)) {
				...
				if (!existsSync(filePath)) {
					...
					created.push(`.opencub/commands/${filename}`);
				}
			}
