...
import {logError, logInfo} from '@/shared/utils/message-queue';
...
export const updateCommand: Command = {
	name: 'update',
	description: 'Update OpenCub to the latest version',
	handler: async (_args: string[]) => {
		...
		try {
			...
			if (updateInfo.hasUpdate) {
				...
				logInfo(
					'Downloading and installing the latest OpenCub update...',
					true,
				);
				...
				if (updateInfo.updateCommand) {
					try {
						...
						return React.createElement(SuccessMessage, {
							message:
								'OpenCub has been updated to the latest version. Please restart your session to apply the update.',
							hideBox: true,
						});
					} catch (err) {
						const errorMessage =
			...
			logError(`Failed to update OpenCub: ${errorMessage}`, true);
			return React.createElement(ErrorMessage, {
