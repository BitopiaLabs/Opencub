...
import {setToolManagerGetter} from '@/shared/message-handler.js';
...
test('updateCommand: runs update command when installed via npm', async t => {
	...
	setToolManagerGetter(() =>
		mockToolManager(async ({command}: {command: string}) => {
			called = true;
			t.is(command, 'npm update -g opencub');
			return 'ok';
		}),
	);
...
test('updateCommand: detects homebrew "not found" error', t => {
	const output =
		'EXIT_CODE: 1\nSTDERR:\nError: opencub not found in Homebrew\nPlease install it first with: brew install opencub';
	t.true(hasCommandFailed(output));
});
